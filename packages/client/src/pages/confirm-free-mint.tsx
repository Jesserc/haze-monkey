...
import { HOLDERS_SNAPSHOT } from '../data'

const ConfirmPresale = () => {
  const [walletAddress, setWalletAddress] = useState('')
  const [confirmFreeMint, setWalletIsWL] = useState<null | boolean>(null)
  ...
  const onFormSubmit = (e: any) => {
    ...
    try {
      const isAvailable = HOLDERS_SNAPSHOT.find(
        (ad) => ad.toLowerCase() === input.toLowerCase()
      )
      setWalletIsWL(Boolean(isAvailable))
    } catch (error) {}
  }
  ...
  return (
    <>
      <SEOHead title="Confirm Free Mint" ogImage="/images/logo.svg" />
            ...
            <div className="flex flex-col items-center text-center">
              ...
              {confirmFreeMint && (
                <p className="mb-6 text-green-500">
                  Wallet is eligible for free mint 🎉🎉🎉
                </p>
              )}

              {confirmFreeMint === null && (
                <p className="mb-6">
                  Confirm if your wallet address is eligible for free mint
                </p>
              )}
